...
const variantStyles = {
  default: 'bg-neu-bg shadow-neu',
  elevated: 'bg-neu-bg shadow-neu-lg',
  bordered: 'bg-neu-bg shadow-neu-sm',
  'gold-accent': 'bg-neu-bg shadow-neu border-l-4 border-secondary-500 pl-6',
}
...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ variant = 'default', clickable = false, padding = 'md', className = '', children, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={`rounded-2xl transition-all duration-200 ${variantStyles[variant]} ${paddingStyles[padding]} ${clickable ? 'cursor-pointer hover:shadow-neu-lg' : ''} ${className}`}
        {...props}
      >
        {children}
      </div>
